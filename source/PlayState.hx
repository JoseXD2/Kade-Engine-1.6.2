...
class PlayState extends MusicBeatState
{
	...
	var funneEffect:FlxSprite = new FlxSprite(1091, 822); //1091 738 
	var NameGF:FlxSprite = new FlxSprite(678, 588).loadGraphic(Paths.image('GF_MELON3version_text', 'shared'));
	var VoiceBox:FlxSprite = new FlxSprite(719, 540).loadGraphic(Paths.image('Voice_box_GF', 'shared'));
    var Box:FlxSprite = new FlxSprite(660, 500); //660 450
	var Xob:FlxSprite = new FlxSprite(5, 350);
	var Cat:FlxSprite = new FlxSprite(350, 400);
				...
				case 'hstage':
					{
						defaultCamZoom = 0.9;
						curStage = 'hstage';
						var bg:FlxSprite = new FlxSprite(-150, 30).loadGraphic(Paths.image('bg6', 'shared'));
				...
				case 'hstageCOLD':
					{
						defaultCamZoom = 0.9;
						curStage = 'hstageCOLD';
						var bg:FlxSprite = new FlxSprite(-155, 30);
			            bg.frames = Paths.getSparrowAtlas('bg_COLD', 'shared');
			            bg.animation.addByPrefix('bg', 'bg_animn', 16);
						...
						var snLw:FlxSprite = new FlxSprite(-152, -2300);
		                snLw.frames = Paths.getSparrowAtlas('snLw', 'shared');
		                snLw.animation.addByPrefix('leaves', 'snow', 24);
		                snLw.animation.play('leaves');
		                snLw.scrollFactor.set(0.9, 0.9);
	                   	snLw.cameras = [camHUD];
		                add(snLw);
						
						var front:FlxSprite = new FlxSprite(-140, 200).loadGraphic(Paths.image('frontCOLD', 'shared'));
						front.scrollFactor.set(0.9, 0.9);
						add(front);
						
						var gift:FlxSprite = new FlxSprite(340, 590);
						gift.frames = Paths.getSparrowAtlas('gift_stage', 'shared');
						gift.animation.addByPrefix('grit', 'gifts', 24);
						gift.animation.play('grit');
						gift.scrollFactor.set(0.9, 0.9);
						add(gift);
						
						var snow:FlxSprite = new FlxSprite(-90, -1800);
						snow.frames = Paths.getSparrowAtlas('snIw', 'shared');
						snow.animation.addByPrefix('leaves', 'snow', 24);
						snow.animation.play('leaves');
	...
	{		
		remove(dad); 
		dad = new Character(230, 470, 'ragdollerror');
		add(dad);
	}
	
	function BewSprite():Void
	{		
		remove(dad);
	    dad = new Character(228, 470, 'errorgmod');
	    add(dad);
	}
	...
	function GfVoiceBox():Void  // ITS WORKS, BUT BAD, SORRY NO COOL FUNCTION FOR YA
	{
		funneEffect.frames = Paths.getSparrowAtlas('Box_gf', 'shared', true);
		funneEffect.animation.addByIndices('GF_TALK', 'voice_gf_new', [3, 2, 1, 0, 4, 5, 6, 7], "", 24, false);
		funneEffect.animation.addByPrefix('GF_TALK_FADE', 'voice_gf_fade', 24, false);
		funneEffect.animation.addByIndices('GF_TALK_FADE_OUT', 'voice_gf_fade', [9, 8, 7, 6, 5, 4, 3, 2, 1, 0], "", 24, false);
		funneEffect.visible = true;
		VoiceBox.visible = true;
		add(funneEffect);
		if (curSong == 'error')
		{
			funneEffect.animation.play('GF_TALK');
			if (curBeat == 0)
			{
			    VoiceBox.visible = false;	
			}
			if (curBeat >= 238)
			{
				funneEffect.animation.play('GF_TALK_FADE');
			    VoiceBox.visible = false;   
			}
		}
	}	
	/*function Talkgf():Void     
	{
		funneEffect.frames = Paths.getSparrowAtlas('Box_gf', 'shared', true);
		funneEffect.animation.addByIndices('GF_TALK', 'voice_gf', [3, 2, 1, 0, 4, 5, 6, 7], "", 44, false);
		funneEffect.animation.addByPrefix('GF_TALK_FADE', 'voice_gf_fade', 24, false);
		funneEffect.animation.play('GF_TALK');
		funneEffect.visible = true;
		if (curBeat == 238)
	...
	function Rem():Void
	{
		Box.frames = Paths.getSparrowAtlas('Box', 'shared', true);
		//BfBox.animation.addByPrefix('BF_TALK', 'bf_box', 24, false);  fuuuuuuuuuk
		Box.animation.addByPrefix('kill', 'killing', 36, false);
		Box.animation.play('kill');
	   ...
	   // {
		 //  BfBox.visible = true;
		 //  BfBox.animation.play('BF_TALK'); 
	   // }
	}
	
	function Xb():Void
	{
		Xob.frames = Paths.getSparrowAtlas('final_thanks', 'shared', true);
		Xob.animation.addByPrefix('text_chat', 'tex_box_anim', 12, false);
		Xob.animation.play('text_chat');
		Xob.scrollFactor.set(0.9 , 0.9);
		Xob.visible = true;
		add(Xob);	
		Xob.cameras = [camHUD];
		//FlxG.sound.play(Paths.sound('angry_typing'));
	}
	
	function Bx():Void
	{
		Cat.frames = Paths.getSparrowAtlas('Change_the_model', 'shared', true);
		Cat.animation.addByPrefix('EndModel', 'Change_Box', 18, false);
		Cat.animation.play('EndModel');
		Cat.scrollFactor.set(0.9 , 0.9);
		Cat.visible = true;
		add(Cat);	
	}
		...
		if (curSong == 'error')
		{
			switch (curBeat)
			{
				...
				case 232:
					NewSprite(); */ 
				case 236:
					FlxG.sound.play(Paths.sound('gmod_ambience'));
					Xb();
				case 240:
					FlxG.sound.play(Paths.sound('gmod_z'));
		            remove(dad);
					Rem();
					//remove(funneEffect);
